...
// set to version 2 to store .jpg file in the specific folder of cloudinary
...
router.post('/setPetPic', upload.single('image'), async (req, res) => {
	...
	try {
		...
		// Convert buffer to base64 and create proper data URI
		const mimeType = req.file.mimetype; // usually 'image/jpeg' or 'image/png'
		const buf64 = req.file.buffer.toString('base64');
		const dataUri = `data:${mimeType};base64,${buf64}`;

		// Upload to Cloudinary (await version)
		const result = await cloudinary.uploader.upload(dataUri, {
			public_id: image_uuid,
			folder: "cloudinaryLab/pets"
		});
		console.log("Cloudinary upload result:", result);

		// Save image UUID to the pet document
		const updateResult = await petCollection.updateOne(
		...
		if (!updateResult.modifiedCount) {
			...
			return res.render('error', { message: 'Error uploading pet image to MongoDB' });
		}

		res.redirect(`/showPets?id=${user_id}`);
	} catch (err) {
		console.error("Error during image upload:", err);
		res.render('error', { message: 'Image upload failed' });
	}
});
...
router.post('/setUserPic', upload.single('image'), async (req, res) => {
	...
	try {
		...
		res.redirect(`/showUsers?id=${user_id}`);
	} catch (err) {
		console.error("Error during image upload:", err);
		res.render('error', { message: 'Image upload failed' });
	}
...
router.get('/showUsers', async (req, res) => {
	...
	try {
		...
		const users = await userCollection.find({ "_id": new ObjectId(user_id) }).toArray();
		if (users === null) {
			res.render('error', { message: 'Error connecting to MongoDB' });
			console.log("Error connecting to userModel");
		}
			...
			console.log(users);
			res.render('index', { allUsers: users, user_id: user_id });
		}
	}
	catch (ex) {
		res.render('error', { message: 'Error connecting to MongoDB' });
